...
### Performing Failback/Reprotect on PowerScale

Failback/Reprotect one-step operations are not natively supported on PowerScale, and are performed as a series of steps in CSM. When any of thse operations are triggered, through the use of `repctl` or by editing the RG, the steps below are performed on the PowerScale storage arrays.
...
2. Performing `Actions > Disallow writes` on the target-side Local Target policy that matches the SyncIQ policy undergoing failback. 
3. Enabling the source-side SyncIQ policy. 
4. Editing the SyncIQ policy's schedule from `Manual` to `When source is modified` and setting the time delay for synchronization as appropriate.
...
8. Editing the SyncIQ policy on the source PowerScale array's schedule the SyncIQ policy's schedule from `Manual` to `When source is modified` and setting the time delay for synchronization as appropriate.

#### Reprotect - Set Original Target as New Source 

A reprotect operation is, in essence, doing away with the original source-target relationship and establishing a new one in the reverse direction. To accomplish this, CSM performs the following steps:
